...
function StatCard({ target, suffix, label, inView }) {
  ...
  return (
    <div className="bg-navy-700 rounded-xl border border-white/8 py-6 px-4 text-center hover:border-gold-500/30 transition-colors">
...
export default function Partners() {
  ...
  return (
    <section className="bg-[#0a0f1a] py-16 px-4 sm:px-6 lg:px-8 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <Animate variant="fade-up">
          <p className="text-center font-heading font-bold text-xs tracking-widest uppercase text-white/30 mb-10">
          ...
          <div className="overflow-hidden">
            <div className="flex gap-4 animate-marquee" style={{ width: 'max-content' }}>
              {[...PARTNERS, ...PARTNERS].map((p, i) => (
                <div key={`${p.id}-${i}`}
                  className="flex-shrink-0 flex items-center gap-3 bg-navy-700 border border-white/8 rounded-xl px-5 py-3.5 hover:border-gold-500/40 hover:bg-navy-600 transition-all duration-200 group">
                  ...
                  <div>
                    <div className="font-heading font-bold text-white text-sm group-hover:text-gold-400 transition-colors whitespace-nowrap">{p.name}</div>
                    <div className="text-white/35 text-xs">{p.sector}</div>
                  </div>
                </div>
              ))}
            </div>
